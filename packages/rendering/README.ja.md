...
| `onReelStop(callback: (reel: ReelPosition, symbol: SymbolSpec) => void)` | 各リール停止時のコールバックを登録 |
| `skipToResult()` | アニメーションをスキップして即時停止 |
| `resolveReach()` | リーチ演出後に中リールを再開（リーチ演出フェーズ以外ではno-op） |
...
| `enableReachPresentation` | false | リーチ演出フェーズを有効化（中リール停止前に一時停止） |
| `pseudoStopDuration` | 400 | 擬似連の停止フリーズ時間（ms） |
| `pseudoRestartDuration` | 600 | 擬似連の再回転加速時間（ms） |
...
リーチ時は `stopping-center` フェーズでイージング付きの長いスローダウンが適用されます。`enableReachPresentation` を有効にすると、リーチ時に `reach-presentation` フェーズで一時停止し、`resolveReach()` が呼ばれるまで待機します。擬似連（`pseudoCount`）が設定されている場合、リーチ前に停止・再回転サイクルが挿入されます。
...
| `ReelPosition` | リール位置：`"left" \| "center" \| "right"` |
